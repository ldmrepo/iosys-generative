...
### 1.1 목적
Qwen3-VL-Embedding-2B 모델을 사용하여 교육 문항의 멀티모달(텍스트+이미지) 임베딩 및 유사 문항 검색 성능을 검증하는 POC(Proof of Concept) 수행.

### 1.2 데이터 현황
- **전체 데이터**: 176,443건 (6개 과목)
- **POC 테스트**: 100건 (수학)
...
│   └── HANDOVER.md              # 본 문서
├── data/
│   ├── raw/                      # 원본 IML 파일 (184,761개)
│   └── processed/                # 전처리된 문항 (176,443건, 18파트)
...
#### Image GT 기준 (이미지 유사도, 27개 쿼리)

| Metric | 2B 멀티모달 | 2B 텍스트 | 8B 텍스트 | Best |
|--------|------------|----------|----------|------|
| Top-1 | **24.9%** | 22.1% | 20.0% | 2B 멀티모달 |
| Top-3 | 51.4% | 45.9% | **55.5%** | 8B 텍스트 |
| Top-5 | **76.3%** | 67.4% | 69.6% | 2B 멀티모달 |
| Top-10 | **94.1%** | 84.0% | 86.2% | 2B 멀티모달 |
| MRR | **90.7%** | 88.1% | 84.5% | 2B 멀티모달 |

#### Hybrid GT 기준 (텍스트 유사도, 61개 쿼리)

| Metric | 2B 멀티모달 | 2B 텍스트 | 8B 텍스트 | Best |
|--------|------------|----------|----------|------|
| Top-1 | **20.9%** | 17.6% | 16.8% | 2B 멀티모달 |
| Top-3 | **64.3%** | 59.0% | 54.9% | 2B 멀티모달 |
| Top-5 | 76.6% | **83.7%** | 71.9% | 2B 텍스트 |
| Top-10 | 85.8% | **100.0%** | 83.9% | 2B 텍스트 |
| MRR | **53.2%** | 49.6% | 44.6% | 2B 멀티모달 |

#### 핵심 발견

| 비교 | 8B 텍스트 | 2B 최고 | 변화 |
|------|----------|---------|------|
| Image GT Top-5 | 69.6% | **76.3%** (멀티모달) | **-6.6%p** |
| Hybrid GT Top-5 | 71.9% | **83.7%** (텍스트) | **-11.9%p** |

#### 원인 분석

1. **모델 차이**: `Qwen3-Embedding-8B` (텍스트 전용) 사용
   - `Qwen3-VL-Embedding-8B` (멀티모달)가 아님

2. **이미지 정보 부재**: 8B 텍스트 모델은 이미지 활용 불가
   - Image GT에서 2B 멀티모달 대비 열세

3. **한국어 최적화**: 2B 모델이 한국어 수학 문항에 더 적합

#### 결론 및 권장사항

| GT 유형 | 최고 성능 모델 | Top-5 Recall |
|---------|---------------|--------------|
| Image GT | **2B 멀티모달 (mean pooling)** | **76.3%** |
| Hybrid GT | **2B 텍스트 전용** | **83.7%** |
...
| 유효 문항 | 131,460개 (74.5%) |
| 이미지 포함 | 45,368개 |
| 출력 파트 | 18개 (items_part01~18.json) |
...
#### 관련 파일
- 전처리 데이터: `data/processed/items_part*.json` (18파트)
- 전처리 통계: `preprocessing/output/statistics.json`
...
| v1.7.0 | 2026-01-28 | 8B vs 2B 성능 비교 평가 완료 | AI TF |
| | | - Image GT: 8B(69.6%) < 2B멀티모달(76.3%) **-6.6%p** | |
...
| v1.9.0 | 2026-01-28 | 전체 과목 데이터 전처리 완료 | AI TF |
| | | - IML 파일 184,761개 → 176,443건 파싱 성공 | |
| | | - 6개 과목: 수학(45.7%), 과학(19.9%), 영어(9.7%), | |
