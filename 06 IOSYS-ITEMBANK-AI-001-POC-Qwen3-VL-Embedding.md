...
**가정**:
- 테스트 데이터 100건이 전체 문항의 특성을 대표함
- 개발 서버 GPU (RTX 3090 24GB) 사용 가능
- 테스트 기간 중 모델 업데이트 없음

**제약**:
- POC 기간: 최대 2주
- 인력: 개발자 1명 + 콘텐츠 담당자 1명
- 예산: 추가 인프라 비용 없음 (기존 장비 활용)
...
| 구성 요소 | 사양 | 비고 |
|----------|------|------|
| **GPU** | NVIDIA RTX 3090 24GB | 개발 서버 |
| **CPU** | AMD Ryzen 9 5900X | 12코어 24스레드 |
| **RAM** | 64GB DDR4 | |
| **Storage** | NVMe SSD 1TB | 모델 저장용 |
...
huggingface-cli download google/siglip-base-patch16-256-i18n --local-dir ./models/siglip-base
...
| R2 | GPU 메모리 부족 | 높음 | 중간 | 배치 크기 축소, FP16 사용 |
...
model = Qwen3VLEmbedder(
    ...
    max_pixels=1048576,                  # 최대 픽셀 제한
    device_map="auto"
)

# 배치 크기 조절
BATCH_SIZE = 8  # 메모리 부족 시 축소
```
